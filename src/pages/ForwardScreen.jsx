...
const ForwardScreen = ({socket,messagesRef,host,saveMessage,db,storeMessageInSQLite,setUsersMain}) => {
  ...
  useEffect(() => {
    const userData = JSON.parse(localStorage.getItem('usersMain')) || [];
    setUsers(userData);
...
async function encryptMessageHybrid(newMessage, recipientPublicKeyPem) {
  ...
  const uploadFile = async (message,token) => {
    if (!message) return;
...
formData.append('filename', message.file_name);
formData.append('fileSize', message.file_size);
